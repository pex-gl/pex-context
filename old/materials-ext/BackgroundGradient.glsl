  ...
  void main() {
    ...
    float dist = 2.0*length(vTexCoord - vec2(0.5, 0.5));
    if (dist > 1.0) dist = 1.0;
    if (labMixing == 0.0) gl_FragColor.rgb = mixrgb(bottomColor.rgb * bottomColor.a, topColor.rgb * topColor.a, dist);
    else gl_FragColor.rgb = mixlab(bottomColor.rgb * bottomColor.a, topColor.rgb * topColor.a, dist);
