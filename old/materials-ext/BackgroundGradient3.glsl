...
void main() {
  ...
  void main() {
    ...
    float value = texture2D(mask, vTexCoord).r;

    float r = rand(vTexCoord);
    if (labMixing == 0.0) gl_FragColor.rgb = mixrgb(bottomColor.rgb, topColor.rgb, value);
    else gl_FragColor.rgb = mixlab(bottomColor.rgb, topColor.rgb, value);
