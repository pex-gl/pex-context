...
function HEFace(hemesh) {
  ...
  this.markers = new Array(HE_MAX_VERT);  
}
...
plask.simpleWindow({
  ...
  iterate: function() {    
    ...
    for(var faceIndex=0; faceIndex < hemesh.faces.length; faceIndex++) {
    ...
    for(var faceIndex=0; faceIndex < hemesh.faces.length; faceIndex++) {
      var face = hemesh.faces[faceIndex];
      for(var slot=0; slot < face.vertexCount; slot++) {
        if (face.markers[slot] != -1) continue; //already processed edge
        
        var edgeLabel = face.labels[slot];
        for(var j=0; j<rules.length; j++) {
          var rule = rules[j];
          if (rule[0] == edgeLabel) { //rule matches edge
