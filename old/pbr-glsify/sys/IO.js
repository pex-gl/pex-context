...
var PlaskIO = function() {
  ...
  IO.loadBinaryFile = function (file, callback) {
    var fullPath = path.resolve(IO.getWorkingDirectory(), file);
    if (!fs.existsSync(fullPath)) {
      if (callback) {
        return callback(null);
...
var WebIO = function () {
  ...
  IO.loadBinaryFile = function (url, callback) {
    var request = new XMLHttpRequest();
    request.open('GET', url, true);
    ...
    request.onreadystatechange = function (e) {
      if (request.readyState == 4) {
        if (request.status == 200) {
          ...
          Log.error('WebIO.loadTextFile error : ' + request.response);
        }
      }
    };
    request.send(null);
