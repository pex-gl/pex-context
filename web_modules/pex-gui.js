import { m as map, c as clamp } from './_chunks/utils-Bi3aYt1K.js';
import { q as toHex, t as toHSL, f as fromHSL } from './_chunks/hsl-E15bvNol.js';
...
class CanvasRenderer {
    draw(items) {
        ...
        for(let i = 0; i < items.length; i++){
            ...
            if (tabs.length > 0) {
                const prevTabs = items.filter((param, index)=>{
                    let { type  } = param;
                    return index < i && type === "tab";
                });
            ...
            if (item.type === "slider") {
                ...
                ctx.fillStyle = this.theme.color;
                ctx.fillText(item.title, x + textPadding, dy + textY);
                Object.entries(item.stats).map((param, i)=>{
...
var _ctx = /*#__PURE__*/ _class_private_field_loose_key$1("_ctx");
class PexContextRenderer extends CanvasRenderer {
    draw(items) {
        super.draw(items);
    }
    afterDraw() {
        _class_private_field_loose_base$1(this, _ctx)[_ctx].update(this.rendererTexture, {
            data: this.canvas,
    ...
    dispose() {
        super.dispose();
        _class_private_field_loose_base$1(this, _ctx)[_ctx].dispose(this.rendererTexture);
    }
    constructor(opts){
        super(opts);
        ...
        const { ctx  } = opts;
        _class_private_field_loose_base$1(this, _ctx)[_ctx] = ctx;
...
var _pixelRatio = /*#__PURE__*/ _class_private_field_loose_key("_pixelRatio"), _scale = /*#__PURE__*/ _class_private_field_loose_key("_scale");
/**
 ...
 */ class GUI {
    ...
    set pixelRatio(ratio) {
        ...
        _class_private_field_loose_base(this, _pixelRatio)[_pixelRatio] = ratio;
    }
    // Helpers
    ...
    onPointerDown(event) {
        ...
        for(let i = 0; i < this.items.length; i++){
            const prevTabs = this.items.filter((param, index)=>{
                let { type  } = param;
                return index < i && type === "tab";
            });
            const parentTab = prevTabs[prevTabs.length - 1];
    ...
    onKeyDown(event) {
        const focusedItem = this.items.filter((param)=>{
            let { type , focus  } = param;
   ...
   */ addTab(title, contextObject, attributeName, options, onChange) {
        ...
        const tab = new GUIControl({
            type: "tab",
            title,
            current: this.items.filter((param)=>{
                let { type  } = param;
                return type === "tab";
            }).length === 0,
            activeArea: [
   ...
   */ addColumn(title, width) {
        if (width === void 0) width = this.theme.columnWidth;
   ...
   */ addStats(title, options) {
        const ctrl = new GUIControl({
            ...
            options: options || {
                update (item) {
                    Object.assign(item.stats, Object.fromEntries(Object.entries(item.ctx?.stats || {}).map((param)=>{
                        let [k, v] = param;
                        return [
                            k,
                            `${v.alive} / ${v.total}`
    ...
    getScaledActiveArea(activeArea) {
        return activeArea.map((a)=>a.map((b)=>b * _class_private_field_loose_base(this, _scale)[_scale]));
    }
    update() {
   ...
   */ draw() {
        ...
        if (this.isAnyItemDirty(this.items) || resized || this.renderer.dirty) {
            ...
            if (this.responsive) {
                _class_private_field_loose_base(this, _scale)[_scale] = Math.min(Math.min(this.canvas.clientWidth / rendererWidth, this.canvas.clientHeight / rendererHeight), this.scale);
            } else {
                _class_private_field_loose_base(this, _scale)[_scale] = this.scale;
            }
        }
        this.drawTexture2d({
            texture,
            rect: [
                0,
                0,
                canvasScale * _class_private_field_loose_base(this, _scale)[_scale] * rendererWidth || 2,
                canvasScale * _class_private_field_loose_base(this, _scale)[_scale] * rendererHeight || 2
            ]
        });
        this.drawTextures();
    }
    drawTextures() {
        ...
        for(let i = 0; i < this.items.length; i++){
            ...
            const scale = _class_private_field_loose_base(this, _scale)[_scale] * this.getScale();
            let bounds = [];
            const drawTexture = (param)=>{
                let { activeArea , texture  } = param;
   ...
   */ constructor(ctx, { pixelRatio =devicePixelRatio , theme ={} , scale =1 , responsive =true , renderer  } = {}){
        ...
        this.ctx = ctx;
        _class_private_field_loose_base(this, _pixelRatio)[_pixelRatio] = pixelRatio;
        this.theme = {
            ...DEFAULT_THEME,
            ...theme
        };
        this.scale = scale;
        _class_private_field_loose_base(this, _scale)[_scale] = scale;
        ...
        this.renderer = renderer || new (isPexContext ? PexContextRenderer : CanvasRenderer)({
            ...
            pixelRatio: _class_private_field_loose_base(this, _pixelRatio)[_pixelRatio],
            theme: this.theme
        });
        if (isPexContext) {
            const attributes = {
            ...
            this.drawTexture2d = (param)=>{
                let { texture , rect , flipY  } = param;
            ...
            this.drawTextureCube = (param)=>{
                let { texture , rect , level , flipEnvMap  } = param;
