...
import './common/classof-f879816f.js';
 ...
 * @private
 */
function computePlane(geometry, indices, su, sv, nu, nv, direction = "z", pw = 0, quads = false, uvScale = [1, 1], uvOffset = [0, 0], center = [0, 0, 0], ccw = true) {
  const {
    positions,
    normals,
    uvs,
...
function roundedRectangle({
  ...
  const uvOffsetCorner = (su, sv) => [[radius / (su + r2), 0], [1 - radius / (su + r2), 0], [1, 1 - radius / (sv + r2)], [0, 1 - radius / (sv + r2)]];
  const uvOffsetStart = (_, sv) => [0, radius / (sv + r2)];
  const uvOffsetEnd = (su, sv) => [1 - radius / (su + r2), radius / (sv + r2)];
  const [su, sv, nu, nv, direction, pw, uvScale, uvOffset, center] = [widthX, widthY, nx, ny, "z", 0, [faceSX, faceSY], [radiusSX, radiusSY], (x, y) => [x, y, 0]];
  ...
  for (let i = 0; i < 4; i++) {
    ...
    // Corners
    computePlane(geometry, indices, radius, radius, roundSegments, roundSegments, flip ? "-z" : "z", pw, false, [(flip ? -1 : 1) * (radius / (su + r2)), radius / (sv + r2)], uvOffsetCorner(su, sv)[i], center(x, y), !flip);
...
function squelched({
  ...
  uvs[index] = [HALF_PI, TAU - HALF_PI].includes(t) ? 0.5 : remap(u / Math.sqrt(1 - v ** 2));
  uvs[index + 1] = [0, TAU, Math.PI].includes(t) ? 0.5 : remap(v / Math.sqrt(1 - u ** 2));
}
function squelchedVertical({
  uvs,
  index,
  u,
  v,
...
function reuleux({
  ...
  return ellipse({
    ...
    equation: ({
      ...
      ry,
      t
    }) => [rx * (cosN * Math.cos(0.5 * (t + PIoverN * (2 * Math.floor(n * t / TAU) + 1))) - Math.cos(PIoverN * (2 * Math.floor(n * t / TAU) + 1))), ry * (cosN * Math.sin(0.5 * (t + PIoverN * (2 * Math.floor(n * t / TAU) + 1))) - Math.sin(PIoverN * (2 * Math.floor(n * t / TAU) + 1)))]
  });
}
