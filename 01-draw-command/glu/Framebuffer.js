...
function Framebuffer(gl, width, height, options) {
  ...
  if (options.color === true) { //make our own
    var texture = Texture2D.create(gl, width, height, options);
...
Framebuffer.prototype.checkExtensions = function() {
  var gl = this.gl;
  if (Platform.isBrowser) {
    if (this.colorAttachments.length > 1) {
      ...
      if (!this.webglDrawBuffersExt) {
        throw new Error('Framebuffer creating multiple render targets:' + this.colorAttachments.length + ' but WEBGL_draw_buffers is not available');
      }
    }
  }
}
...
Framebuffer.prototype.bindAndClear = function () {
  var gl = this.gl;
  this.bind();
...
Framebuffer.prototype.checkFramebuffer = function() {
  var gl = this.gl;
...
Framebuffer.prototype.getDepthAttachement = function() {
